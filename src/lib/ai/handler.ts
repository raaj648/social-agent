...
export async function handleAIResponse(
  ...
  try {
    ...
    if (deductError) {
      ...
      console.warn(`[handler] User ${targetUserId} insufficient points (needed ${creditsToDeduct})`);
    }
    ...
    await supabase.from('usage_logs').insert({
      ...
      metadata: {
        ...
        voice_duration_seconds: actionType === 'voice_read' ? voiceDurationSeconds ?? null : null,
      },
    });
  } catch (error) {
