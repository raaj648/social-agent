-- Final migration: complete tenant removal
...
-- Set user_id values using channel references
update public.conversations c set user_id = coalesce((select user_id from public.connected_pages where connected_pages.id = c.page_id limit 1),(select user_id from public.instagram_accounts where instagram_accounts.id = c.instagram_id limit 1),(select user_id from public.whatsapp_accounts where whatsapp_accounts.id = c.whatsapp_id limit 1)) where user_id is null;
update public.ai_settings a set user_id = coalesce((select user_id from public.connected_pages where connected_pages.id = a.page_id limit 1),(select user_id from public.instagram_accounts where instagram_accounts.id = a.instagram_id limit 1)) where user_id is null;
update public.knowledge_base kb set user_id = (select user_id from public.connected_pages limit 1) where user_id is null;
update public.orders o set user_id = (select user_id from public.connected_pages limit 1) where user_id is null;
...
-- Drop tenants table and old functions
